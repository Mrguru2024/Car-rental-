...
import { validateBookingEligibility, type BookingEligibilityInput } from '@/lib/vehicle-tiers'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const renterProfileId = renter_id
      ? renter_id
      : (await supabase.from('profiles').select('id').eq('user_id', user.id).single()).data?.id
    ...
    const { data: vehicle, error: vehicleError } = await adminSupabase
      .from('vehicles')
      .select('id, year, vehicle_tier, title_type, inspection_status, status, dealer_id')
      .eq('id', vehicle_id)
      .single()
    ...
    try {
      ...
      const advisory =
        (complaintCount || 0) > 2 || (avgRating < 2.5 && (reviews?.length || 0) > 3)
          ? 'watchlisted'
          : 'none'

      isFlagged = advisory === 'watchlisted' || advisory === 'restricted'
    ...
    const eligibilityInput: BookingEligibilityInput = {
      ...
      dealerPolicy: dealerPolicy
        ? {
            ...
            min_renter_standing_grade: dealerPolicy.min_renter_standing_grade as any,
            ...
            require_soft_credit_for_tier3: dealerPolicy.require_soft_credit_for_tier3,
            require_soft_credit_for_tier4: dealerPolicy.require_soft_credit_for_tier4,
