/**
 * Prime Admin Dispute Override Endpoint
 * POST /api/prime-admin/disputes/:id/decision
 */
...
export async function POST(
  ...
  try {
    ...
    // Validate decision (prime admin override decisions)
    ...
    // Validate transition (prime admin can override more things)
    const validation = validateStatusTransition(profile.role as any, current.status as any, newStatus)
    if (!validation.allowed) {
      return NextResponse.json({ error: validation.reason }, { status: 400 })
    }

    // Create decision record (append, don't delete previous decisions)
    ...
    if (decisionError || !decisionRecord) {
      console.error('Failed to create override decision:', decisionError)
      return NextResponse.json({ error: 'Failed to create override decision' }, { status: 500 })
    }
    ...
    await logAuditEvent({
      ...
      action: 'DISPUTE_PRIME_ADMIN_OVERRIDE',
    ...
    console.error('Prime admin dispute override error:', error)
