...
export const metadata: Metadata = {
  ...
  description: 'Browse available vehicles for rent on Drivana. Find the perfect car, truck, or SUV from trusted local dealers in Atlanta.',
  openGraph: {
    title: 'Browse Vehicles | Drivana',
    description: 'Browse available vehicles for rent on Drivana. Find the perfect car, truck, or SUV from trusted local dealers in Atlanta.',
    type: 'website',
    siteName: 'Drivana',
  },
...
export default async function ListingsPage({
  ...
  const { location, start_date, end_date, min_mileage_limit, max_mileage_limit } = params
  
  // Normalize max_mileage_limit - treat "unlimited" or empty as no max filter
  const hasMaxLimit = max_mileage_limit && max_mileage_limit !== 'unlimited' && max_mileage_limit !== ''
  ...
  if (start_date && end_date && displayVehicles.length > 0) {
    ...
    const vehicleIds = displayVehicles.map((v: any) => v.id).filter((id: string) => !id.startsWith('placeholder') && !id.startsWith('seed-'))
    ...
    if (vehicleIds.length > 0) {
      ...
      filteredVehicles = displayVehicles.filter((v: any) => !unavailableVehicleIds.has(v.id))
    }
    ...
    filteredVehicles = filteredVehicles.filter((v: any) => {
      ...
      if (min_mileage_limit) {
        ...
        if (!isNaN(minMiles) && minMiles > 0) {
          ...
          if (mileageLimit !== null && mileageLimit !== undefined && mileageLimit < minMiles) {
            return false
          }
        }
      }
      ...
      if (hasMaxLimit) {
        ...
        if (!isNaN(maxMiles)) {
          ...
          if (mileageLimit !== null && mileageLimit !== undefined && mileageLimit > maxMiles) {
            return false
          }
        }
      }
  ...
  return (
    <div className="min-h-screen bg-brand-white dark:bg-brand-navy text-brand-navy dark:text-brand-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        {/* Search Bar */}
        <div className="bg-white dark:bg-brand-navy-light rounded-lg shadow-md dark:shadow-brand-navy/30 p-4 sm:p-6 mb-6 sm:mb-8 border border-brand-white dark:border-brand-navy/50 overflow-hidden w-full">
          <form action="/listings" method="get" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4 w-full max-w-full">
            <div className="sm:col-span-2 lg:col-span-1 w-full min-w-0">
              <label htmlFor="search-location" className="block text-sm font-medium text-brand-navy dark:text-brand-white mb-1.5">
                Location
              </label>
            ...
            <div className="w-full min-w-0">
              <label htmlFor="search-start-date" className="block text-sm font-medium text-brand-navy dark:text-brand-white mb-1.5">
                Start Date
              </label>
              <input
            ...
            <div className="w-full min-w-0">
              <label htmlFor="search-end-date" className="block text-sm font-medium text-brand-navy dark:text-brand-white mb-1.5">
                End Date
              </label>
              <input
                id="search-end-date"
            ...
            <MileageSlider minMileage={min_mileage_limit} maxMileage={max_mileage_limit} />
            <div className="sm:col-span-2 lg:col-span-1 flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full">
              ...
              {(location || start_date || end_date || min_mileage_limit || hasMaxLimit) && (
                <ResetFiltersButton />
              )}
            </div>
          </form>
        ...
        {filteredVehicles && filteredVehicles.length > 0 && (
          <div className="mb-6">
            <p className="text-brand-gray dark:text-brand-white/70">
              ...
              {start_date && end_date && ` from ${new Date(start_date).toLocaleDateString()} to ${new Date(end_date).toLocaleDateString()}`}
            </p>
          </div>
        )}
