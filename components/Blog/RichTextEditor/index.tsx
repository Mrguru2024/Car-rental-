          ...
          fallback={
            <div className="min-h-[400px]">
              ...
              <p className="mt-2 text-xs text-brand-gray dark:text-brand-white/50">
                Note: Rich text editor unavailable. Using plain text editor. You
                can use HTML tags for formatting.
              </p>
            </div>
          }
          onError={(error) => {
            // Suppress findDOMNode and Quill range errors - they're expected with react-quill
            const errorMessage = error?.message || "";
            const errorStack = error?.stack || "";
            if (
              errorMessage.includes("findDOMNode") ||
              errorStack.includes("findDOMNode") ||
              errorMessage.includes("addRange") ||
              errorMessage.includes("The given range isn't in document") ||
              errorStack.includes("addRange")
            ) {
              return false; // Don't show error UI for these non-breaking errors
            }
            return true; // Show error UI for other errors
          }}
        >
          <ReactQuillWrapper
            theme="snow"
